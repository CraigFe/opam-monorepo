...
let calculate_opam ~build_only ~local_opam_files ~local_packages ~versions =
  OpamGlobalState.with_ `Lock_none (fun global_state ->
      OpamSwitchState.with_ `Lock_none global_state (fun switch_state ->
          check_repo_config ~global_state ~switch_state;
          Opam_solve.calculate ~build_only ~local_opam_files ~local_packages ?versions
            switch_state))
...
let run (`Repo repo) (`Recurse_opam recurse) (`Build_only build_only) (`Local_packages lp)
  ...
  calculate_opam ~build_only ~local_opam_files ~local_packages ~versions
...
let versions =
  let doc = "Determined versions to lock with in the lockfile." in
  Common.Arg.named
    (fun x -> `Versions x)
...
let term =
  let open Term in
  term_result
    ( const run $ Common.Arg.repo $ recurse_opam $ build_only $ packages $ versions
